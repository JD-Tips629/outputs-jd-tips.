...
const freeResultsList = document.querySelector("#freeResultsList");
const vipResultsList = document.querySelector("#vipResultsList");
let brandTapCount = 0;
let brandTapTimer;
...
function canSeeVipDetails() {
  return Boolean(state.vip || state.admin);
}
...
function tipCard(tip) {
  const isLocked = tip.access === "vip" && !canSeeVipDetails();
  ...
  const adminActions =
    state.admin && tip.status === "active"
      ? `<div class="result-actions">
          <input class="result-score" data-tip-id="${tip.id}" placeholder="Resultado ex: 3-0" />
...
function renderTips() {
  const visibleTips = getActiveTips().filter((tip) => state.filter === "all" || tip.access === state.filter);
  tipsGrid.innerHTML = visibleTips.length
    ? visibleTips.map(tipCard).join("")
    : `<article class="tips-card"><h3>Nenhum palpite ativo publicado ainda.</h3><p>Quando o admin publicar um novo palpite, aparece aqui.</p></article>`;
...
function resultTip(tip) {
  const isVipLocked = tip.access === "vip" && !canSeeVipDetails();
  const amount = tip.resultAmount ? ` | + ${money(tip.resultAmount)}` : "";
  const score = tip.finalScore ? `Resultado ${tip.finalScore}` : "Resultado final por confirmar";
  const details = isVipLocked
    ? `<p class="vip-result-lock">Metodo VIP protegido. Assina para ver a forma da aposta.</p>`
    : `<p>${tip.market} | Odd ${tip.odd}${amount}</p>`;
  return `
    <div class="feed-item ${isVipLocked ? "vip-result-hidden" : ""}">
      <div>
        <strong>${tip.match}</strong>
        <p>${score}</p>
        ${details}
...
function renderSocialFeed() {
  ...
  const freeResults = results.filter((tip) => tip.access !== "vip");
  const vipResults = results.filter((tip) => tip.access === "vip");
  freeResultsList.innerHTML = freeResults.length
    ? freeResults.map(resultTip).join("")
    : `<div class="feed-item"><p>Nenhum resultado Free marcado ainda.</p></div>`;
  vipResultsList.innerHTML = vipResults.length
    ? vipResults.map(resultTip).join("")
    : `<div class="feed-item"><p>Nenhum resultado VIP marcado ainda.</p></div>`;
}
...
function renderAdmin() {
  ...
  adminResults.innerHTML =
    getResultTips().length === 0
      ...
      : getResultTips()
          .map((tip) => `<div class="admin-item"><strong>${tip.status.toUpperCase()} - ${tip.match}</strong><p>${tip.access.toUpperCase()} | ${tip.finalScore ? `Resultado ${tip.finalScore}` : "Sem placar"} | ${tip.resultAmount ? money(tip.resultAmount) : "Sem valor financeiro"}</p></div>`)
          .join("");
}
...
document.addEventListener("click", async (event) => {
  ...
  if (markResult) {
    ...
    const scoreInput = document.querySelector(`.result-score[data-tip-id="${markResult.dataset.tipId}"]`);
